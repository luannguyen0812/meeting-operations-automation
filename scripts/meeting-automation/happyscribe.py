...
def export_transcript_text(transcription_id):
    """Export transcript as plain text via the async exports jobs API."""
    import time

    resp = requests.post(
        f'{BASE_URL}/exports',
        headers=HEADERS,
        json={'export': {'transcription_ids': [transcription_id], 'format': 'txt', 'type': 'text'}},
        timeout=30,
    )
    resp.raise_for_status()
    ...
    for _ in range(24):  # wait up to ~2 min
        time.sleep(5)
        poll = requests.get(f'{BASE_URL}/exports/{export_id}', headers=HEADERS, timeout=15)
        ...
        if state == 'ready':
            ...
            if not dl:
                raise RuntimeError('Export ready but no download_link')
            dl_resp = requests.get(dl, headers=HEADERS, timeout=60)
            dl_resp.raise_for_status()
            return dl_resp.text
        if state == 'failed':
            raise RuntimeError(f'HappyScribe export failed for {transcription_id}')

    raise TimeoutError(f'HappyScribe export timed out for {transcription_id}')
