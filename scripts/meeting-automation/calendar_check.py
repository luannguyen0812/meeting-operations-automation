...
def get_tomorrows_meetings(target_date=None):
    """Check calendar for meetings on target_date (defaults to tomorrow). Returns matched projects and unmatched events."""
    ...
    tomorrow = target_date if target_date is not None else (now_et + timedelta(days=1)).date()
...
def should_generate_templates(target_date=None):
    """Returns (should_generate, projects, unmatched, date)."""
    projects, unmatched, tomorrow = get_tomorrows_meetings(target_date=target_date)
